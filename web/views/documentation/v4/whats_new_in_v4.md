...
- **Protected and Conserved Area Parcels**: New endpoints for accessing individual parcels within protected and conserved areas
...
### Protected and Conserved Areas Response Changes

#### Fields Removed:
- id → `site_id` - Previously an alias for `wdpa_id`. Use `site_id` in v4
- sub_locations - Sub-location data is no longer captured for protected and conserved areas
...
- `inland_waters` - Information about Information about inland water management measures  (String)
...
- `protected_area_parcels` - Array containing detailed information for all parcels within the protected or conserved area. Returns an empty array if no parcels exist. See Parcel Integration section below for detailed structure
...
#### Protected and Conserved Area Parcels
- **New Resource**: Complete new set of endpoints for accessing individual parcels within protected and conserved areas.
- **Enhanced Detail**: Access granular data for each parcel that makes up a protected or conserved area.
- **Comprehensive Coverage**: All protected and conserved area endpoints now include a `protected_area_parcels` array

#### Parcel Integration
- Protected and conserved area responses now include a `protected_area_parcels` array containing all parcels
- The primary protected or conserved area data represents the first parcel
- The `protected_area_parcels` array includes all parcels (including the first one) for complete coverage
- Each parcel object contains similar structure as the main protected or conserved area object. [See example response structure](/documentation#get-v4protectedareaparcelssiteid)